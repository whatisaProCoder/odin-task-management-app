...
export default function createFilteredPage(pageName) {
    ...
    for (const project of projects) {
        ...
        if (pageName === "High Priority Tasks") {
            ...
            tasksInProject = project.tasks.filter((task) => task.priority == "Medium");
            ...
            tasksInProject = project.tasks.filter((task) => {
                ...
                if (todaysDate.getDate() == taskDate.getDate() && todaysDate.getMonth() == taskDate.getMonth() && todaysDate.getFullYear() == taskDate.getFullYear()) {
                    return true;
                } else {
                    return false;
                }
        ...
        filteredTasks.push({ projectID: project.projectID, projectName: project.projectName, tasks: tasksInProject });
    }
...
function populateFilteredTasks(filteredTasks) {
    ...
    for (const filteredTask of filteredTasks) {
        ...
        for (const taskObject of filteredTask.tasks) {
            filteredTasksContainer.append(createTaskCard(filteredTask.projectID, taskObject));
        }
    }
}